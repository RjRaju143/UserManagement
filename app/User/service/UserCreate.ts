...
export class UserCreationService {
  public async createUser({
    ...
    try {
      ...
      const userGroups = await AppDataSource.manager.find(AuthGroup, {
      ...
      const validUserGroups = await Promise.all(
        validGroupIds.map(async (groupId) => {
          const authGroup = await AppDataSource.manager.findOne(AuthGroup, {
            where: { id: groupId },
          })
          if (authGroup) {
            const userGroup = AppDataSource.manager.create(UserGroup, {
              group: authGroup,
              user: newUserSavedData,
            })
      ...
      const filteredUserGroups = validUserGroups.filter(
        (group): group is { id: number; name: string } => group !== null
      )
