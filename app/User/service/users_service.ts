...
export class UserService {
  ...
  public async create(
    ...
    const { username, password, email, isAdmin, firstname, lastname, phone, gender, groupIds } =
      payload
    try {
      if (user.isSuperuser) {
        return await this.userCreationService.createUser({
          username,
          password,
          email,
          isAdmin,
      ...
      if (user.isAdmin || userPermissions.includes('add_user')) {
        ...
        const invalidGroupIds = groupIds
          .map((id) => Number(id))
  ...
  public async updateUser(
    ...
    try {
      ...
      if (user.isAdmin || userPermissions.includes('change_user')) {
        const parsedGroupIds = Array.isArray(groupIds) ? groupIds.map(Number) : [parseInt(groupIds)]
        ...
        if (commonGroupIds.length === 0) {
          return {
            status: 403,
            message: 'Forbidden: You cannot update this user as they are not in the same group.',
          }
        }
  ...
  public async refreshToken(refresh: string): Promise<RefreshTokenResponse> {
    try {
      const token = await AppDataSource.manager.findOne(AuthToken, {
      ...
      const decoded = jwt.verify(token.refreshToken, jwtConfig.jwt.secret) as {
        id: number
        userType: string
      }
    ...
    try {
      ...
      if (user.isSuperuser) {
        ...
        const groupPermissionsPromises = groups.map(async (g) => {
          return await AppDataSource.manager.find(AuthGroupPermissions, {
            where: { group: { id: g.id } },
          })
        })
  ...
  public async updateGroupById(
    ...
    try {
      if (user.isSuperuser) {
        ...
        const duplicateGroup = await AppDataSource.manager.findOne(AuthGroup, {
        ...
        const validPermissionsPromise = AppDataSource.manager.find(AuthPermission, {
          where: { id: In(permission_ids.map(Number)) },
        })
        const existingPermissionsPromise = AppDataSource.manager.find(AuthGroupPermissions, {
          where: { group: { id } },
        })
        const [validPermissions, existingPermissions] = await Promise.all([
          validPermissionsPromise,
        ...
        const duplicates = permission_ids.filter(
          (id, index) => permission_ids.indexOf(id) !== index
        )
        ...
        const updatedPermissions = await AppDataSource.manager.find(AuthGroupPermissions, {
          where: { group: { id } },
        })
  ...
  public async createGroup({
    ...
    const existingPermissions = await AppDataSource.manager.findByIds(
      AuthPermission,
      permissionsIds
    )
    ...
    await Promise.all(
      validPermissionsIds.map((permissionId) => {
